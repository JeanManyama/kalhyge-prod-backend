...
export default {
	...
	async getAll(_, res) {
		try {
			...
			res
				.status(500)
				.json({ error: "Erreur lors de la récupération des machines." });
		}
	},
	...
	async getOne(req, res, next) {
		...
		try {
			...
			res
				.status(500)
				.json({ error: "Erreur lors de la récupération de la machine." });
		}
	},
	...
	async create(req, res) {
		...
		try {
			...
			if (existingMachine) {
				return res
					.status(409)
					.json({ error: "Une machine avec ce nom existe déjà." });
			}
			...
			if (error.name === "SequelizeUniqueConstraintError") {
				return res
					.status(409)
					.json({ error: "Le nom de la machine existe déjà." });
			}
			res
				.status(500)
				.json({ error: "Erreur lors de la création de la machine." });
		}
	},
