...
,
    ...
    // const newQuantity = Number.parseInt( req.body.quantity_reject_aff);
		...
		const lastQuantity = Number.parseInt(lastReject? lastReject.quantity_reject_aff : 0);
	...
	async updateMachineOrArticleReject (req, res, next)  {
		console.log("Nous sommes rentrer dans updateMachineOrArticleReject------------------------")
			const actionType = req.body.actionType;// 'machine' ou 'article'
			console.log("le type d'action est ---------------------------- : ", actionType);
			 // Vérifier que le champ actionType existe
			 if (!actionType) {
        return res.status(400).json({ error: "Impossible d'effectuer l'operation" });
    }
					...
					where: {
								...
								if(currentRecordTab[0].quantity_reject_aff > newQuantity){
									// Je dois le placer avant, avec la date - 1min
										...
										const newDate = new Date(initialDate.getTime() - 60 * 1000);
											...
											const [, articles] = await Production.update({machine_id : machineId, quantity_reject_aff : newQuantity, created_at :new Date()}, {
												where:  {id: rejectId} ,
												returning: true,
											});
						...
						{ 
							...
							if(dateLast !== 0 && dateNext !== 0){
								...
								const [, articles] = await Production.update({machine_id : machineId, created_at : randomDate  }, {
									where:  {id: rejectId} ,
									returning: true,
								});
						...
						where: {
									...
									if(currentRecordTab[0].quantity_reject_aff > newQuantity){
										// Je dois le placer juste avant, avec la date - 1min
											...
											const newDate = new Date(initialDate.getTime() - 60 * 1000);
												...
												const [, articles] = await Production.update({article_id : articleId, quantity_reject_aff : newQuantity, created_at :new Date()}, {
													where:  {id: rejectId} ,
													returning: true,
												});
							...
							{ 
								...
								if(dateLast !== 0 && dateNext !== 0){
									...
									const [, articles] = await Production.update({article_id : articleId, created_at : randomDate  }, {
										where:  {id: rejectId} ,
										returning: true,
									});
									...
									// La date initiale
										...
										// Je mets toute ligne avec une nouvelle date créee et la nouvelle machine
									const [, articles] = await Production.update({article_id : articleId, created_at : newDate  }, {
										where:  {id: rejectId} ,
										returning: true,
									});
									...
									const [, articles] = await Production.update({article_id : articleId, created_at : new Date() }, {
										where:  {id: rejectId} ,
										returning: true,
									});
									if (!articles || !articles.length) {
	...
	async updateQuantityReject(req, res, next) {
		...
		const currentReject = await Production.findOne({
			...
			attributes: ["machine_id","article_id","quantity_reject_aff","created_at","timer_id"],
		});

		const currentQuantity = Number.parseInt(currentReject?currentReject.quantity_reject_aff : 0);
	...
	// Recuperation de l'enregistrement +1 du reject
		...
		if ((Number.parseInt(newQuantity)===0)||(currentQuantity === Number.parseInt(newQuantity)) || ((nextR !== 0) && (newQuantity >= nextR)) || (newQuantity <= previous)) {
			...
			const [, articles] = await Production.update({quantity_reject_aff : newQuantity }, {
				where:  {id: rejectId} ,
				returning: true,
			});
			if (!articles || !articles.length) {
