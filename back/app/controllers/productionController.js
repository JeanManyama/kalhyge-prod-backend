...
export default {
		...
		function transformData(data) {
			return data.map((item) => {
				...
				const rejects = item.machines.map((machine) => {
					const reject = machine.dataValues.total_quantity_reject?.map(
						Number,
					) || [0]; // Convertir en tableau de nombres
					return reject;
				});

				// Calculer les résultats pour chaque machine
				const total_quantity_all = quantities.reduce(
					(sum, q) => sum + calculateSumOfDifferences(q),
					0,
				);
				const total_quantity_all_reject = rejects.reduce(
					(sum, r) => sum + calculateSumOfDifferences(r),
					0,
				);
				const total_quantity_valid =
					total_quantity_all - total_quantity_all_reject;
	...
	async create(req, res, next) {
		...
		if (lastQuantity < Number.parseInt(newQuantity)) {
			...
			// enregitrement
			const prod = await Production.create(productionInput, {
				returning: true,
			});
			...
			return res
				.status(500)
				.json({ error: "Impossible d'effectuer cette production" });
		}
	},
	...
	async updateMachine(req, res, next) {
		...
		const qantityProd = await Production.findOne({
			where: { id: productionId },
			attributes: ["quantity_product_aff"],
		});
		...
		if (!currentRecord) {
			...
			if (currentRecordTab.length === 1) {
				...
				if (dateLast !== 0 && dateNext !== 0) {
					...
					const [, machines] = await Production.update(
	...
	async delete(req, res, next) {
		...
		const result = await Production.findOne({
			where: {
				id: idProd,
				[Op.or]: [{ quantity_reject_aff: { [Op.ne]: 0 } }],
			},
		});
		if (result) {
			res.status(403).json({
			...
			res
				.status(204)
				.json({ status: 204, message: "suppression avec success" });
		}
	},
	...
	async getByProductionMachine(req, res, next) {
		...
		function transformData(data, res) {
			...
			const quantities = data.articles.map((article) => {
				...
				const quantity = article.dataValues.total_quantity
					? article.dataValues.total_quantity.map(Number)
					: [];
				return quantity;
			});
			...
			const rejects = data.articles.map((article) => {
				...
				const reject = article.dataValues.total_quantity_reject
					? article.dataValues.total_quantity_reject.map(Number)
					: [];
				return reject;
			});
	...
	async getByProductionArticleWithTime(req, res, next) {
		...
		const productionData = await Production.findAll({
			where: {
				article_id: articleId,
				timer_id: timer_id,
				[Op.and]: [{ quantity_product_aff: { [Op.gt]: 0 } }],
			},
			include: [
				{
					model: Machine,
	...
	async getByProductionHistorique(req, res, next) {
		...
		if (!date || !period) {
			return res
				.status(400)
				.json({ status: "error", message: "Date et période sont requises." });
		}
		...
		try {
			...
			console.log(
				"LES TIMERS SELECTIONNES pour cette date sont ------------------:",
				timers,
			);
				...
				if (timerIds.length > 1) {
						...
						include: [
							{
								...
								attributes: [
									...
									[
										Sequelize.fn(
											"ARRAY_AGG",
											Sequelize.col(
												"machines->Production.quantity_product_aff",
											),
										),
										"total_quantity",
									],
									...
									[
										Sequelize.fn(
											"COALESCE",
											Sequelize.col(
												"machines->Production.objective_historical",
											),
											null,
										),
								...
								where: Sequelize.and(
									Sequelize.where(
										Sequelize.fn(
											"DATE",
											Sequelize.col("machines->Production.created_at"),
										),
										"=",
										date,
									), // Filtre sur la date
					...
					function transformArticleData(data) {
						return data.map((item) => {
							...
							const rejects = item.machines.map((machine) => {
								const reject =
									machine.dataValues.total_quantity_reject.map(Number);
								return reject;
							});
						...
						include: [
							{
								...
								attributes: [
									...
									"name",
									[
										Sequelize.fn("COALESCE", Sequelize.col("objective"), 0),
										"objective",
									],
									[
										Sequelize.fn(
											"ARRAY_AGG",
											Sequelize.col(
												"articles->Production.quantity_product_aff",
											),
										),
										"total_quantity",
									],
								...
								where: Sequelize.and(
									Sequelize.where(
										Sequelize.fn(
											"DATE",
											Sequelize.col("articles->Production.created_at"),
										),
										"=",
										date,
									),
					...
					const durationTimer1 =
						timerIds.length > 1
							? await Timer.findOne({
									where: { id: timerIds[0] },
									attributes: ["time_elapsed"],
								})
							: null;
					const durationTimer2 =
						timerIds.length > 1
							? await Timer.findOne({
									where: { id: timerIds[1] },
									attributes: ["time_elapsed"],
					...
					const transformedArticlesTimer1 =
						transformArticleData(articlesTimer1);
					const transformedArticlesTimer2 =
						transformArticleData(articlesTimer2);

					// Transformer les données des machines pour les deux périodes
					const transformedMachinesTimer1 =
						transformMachineData(machinesTimer1);
					const transformedMachinesTimer2 =
						transformMachineData(machinesTimer2);
					...
					function addArticleQuantities(timer1Data, timer2Data) {
						...
						for (const article1 of timer1Data) {
							...
							const article2 = timer2Data.find(
								(article) => article.id === article1.id,
							);
							...
							if (article2) {
								result.push({
									...
									total_quantity_all:
										article1.total_quantity_all + article2.total_quantity_all,
									total_quantity_all_reject:
										article1.total_quantity_all_reject +
										article2.total_quantity_all_reject,
									total_quantity_valid:
										article1.total_quantity_valid +
										article2.total_quantity_valid,
								});
							}
						}
					...
					const sumArticles = addArticleQuantities(
						transformedArticlesTimer1,
						transformedArticlesTimer2,
					);
					...
					function addMachineQuantities(timer1Machines, timer2Machines) {
						...
						for (const machine1 of timer1Machines) {
							...
							const machine2 = timer2Machines.find(
								(machine) => machine.id === machine1.id,
							);
							...
							if (machine2) {
								result.push({
									...
									name: machine1.name,
									total_quantity_all:
										machine1.total_quantity_all + machine2.total_quantity_all,
									total_quantity_all_reject:
										machine1.total_quantity_all_reject +
										machine2.total_quantity_all_reject,
									total_quantity_valid:
										machine1.total_quantity_valid +
										machine2.total_quantity_valid,
								});
							}
						}
					...
					const sumMachines = addMachineQuantities(
						transformedMachinesTimer1,
						transformedMachinesTimer2,
					);
					...
					const durationTimer1 = await Timer.findOne({
						where: { id: timerIds[0] },
						attributes: ["time_elapsed"],
					});
					let totalDuration = 0;
					...
					include: [
						{
							...
							attributes: [
								...
								[
									Sequelize.fn(
										"COALESCE",
										Sequelize.col("machines->Production.objective_historical"),
										null,
									),
									"objective_historical",
								],
				...
				res.json({
					...
					data: {
						...
						productionTime: duration
							? formatDuration(duration.time_elapsed)
							: null,
					},
				});
				//--------------PRODUCTION MATIN-------------------------//
		...
		try {
			...
			if (period === "production") {
				...
				if (timerIds.length > 0) {
					...
					return res
						.status(404)
						.json({ message: "Aucun timer trouvé pour cette période." });
				}
				...
				return res
					.status(404)
					.json({ message: `Aucun timer trouvé pour la période ${period}.` });
			}
			...
			if (timerToDelete && timerToDelete.time_begin === timers[0].time_begin) {
				...
				await Article.update({ objective: 0 }, { where: {} });
			}
