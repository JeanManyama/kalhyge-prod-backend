	  ...
	  // Récupérer tous les timers
		async getAll(_, res) {
			try {
				...
				console.error(err);
				res.status(500).json({ message: "Erreur lors de la récupération des timers" });
			}
		},
...
async create(req, res) {
  try {
    ...
    if (!user_id) {
       res.status(400).json({ message: "L'identifiant de l'utilisateur est requis" });
       return
    }
...
async reset(req, res) {
  try {
    ...
    console.error(err);
    res.status(500).json({ message: "Erreur lors de la réinitialisation du timer" });
  }
}
,
...
async delete(req, res, next) {
	try {
		...
		console.error(err);
		res.status(500).json({ message: "Erreur lors de la suppression du timer" });
	}
},
...
async getActiveTimer(req, res) {
  try {
    ...
    console.error(err);
    res.status(500).json({ message: "Erreur lors de la récupération du timer actif" });
  }
},
...
async  getTodayTimer(req, res) {
  try {
    ...
    if (!activeTimer) {
      res.status(404).json({ message: "Aucun timer trouvé pour aujourd'hui." });
      return;
    }
...
async function updateTimerRealTime() {
  ...
  try {
    ...
    const now = new Date();
    const timeElapsed = Math.floor((now - new Date(activeTimer.time_begin)) / 1000); // Calcul en secondes
