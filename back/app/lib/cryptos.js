...
const cryptos = {
	async hash(password) {
		...
		const { saltLength, hashLength, cost, blockSize, parallelization, maxmem } =
			config.auth.crypto.scrypt;

		const salt = crypto.randomBytes(saltLength).toString("hex");
		const hashBuffer = await scrypt(password, salt, hashLength, {
			cost,
			blockSize,
			parallelization,
			maxmem,
	...
	async compare(plainTextPassword, hashedPassword) {
		...
		const { hashLength, cost, blockSize, parallelization, maxmem } =
			config.auth.crypto.scrypt;
		const plainTextPasswordBuffer = await scrypt(
			plainTextPassword,
			salt,
			hashLength,
